/**
 * BOOM Protocol - Swap E2E Test
 * 
 * Creates a fresh round, seeds the pool with tokens, then tests atomic buy/sell.
 * 
 * Flow:
 * 1. Start presale → deposit → wait cooldown → end presale → mark winner
 * 2. Create token → register presale token → init explosion → create pool
 * 3. Claim winner tokens
 * 4. Direct transfer tokens to vault (with hook accounts) + sync_pool_reserves
 * 5. Fund SOL vault
 * 6. swap_atomic_buy (SOL → tokens)
 * 7. swap_atomic_sell (tokens → SOL)
 */
...
async function main() {
  ...
  console.log('🔄 BOOM Protocol - Swap E2E Test');
  console.log('=================================');
  ...
  if (bal < 1.5 * LAMPORTS_PER_SOL) { console.log('Need >= 1.5 SOL'); return; }
  ...
  try {
    await sendTx(conn, new TransactionInstruction({
      ...
      programId: BOOM_PROGRAM_ID,
      data: Buffer.concat([disc('start_explosion_timer'), new BN(600).toArrayLike(Buffer, 'le', 8)]),
    }), [mainWallet]);
    pass('Start Explosion Timer');
  ...
  try {
    // Transfer some SOL directly to sol_vault so the pool has SOL reserves
  ...
  try {
    ...
    if (e.logs) { const el = e.logs.filter((l: string) => l.includes('Error') || l.includes('failed') || l.includes('custom')); if (el.length) console.log('  Logs:', el.slice(0, 5)); }
    return;
  }
  await sleep(DELAY_MS);

  // ===== 16. SWAP ATOMIC BUY (SOL → tokens) =====
  console.log('\n--- Step 15: Swap Atomic Buy ---');
  
  // Create buyer ATA (use mainWallet as buyer)
  ...
  // Hook PDAs (extraMetasPda already derived above)
  ...
  try {
    ...
    pass('Swap Atomic Buy', `got ${buyerBal.value.uiAmount} tokens for 0.01 SOL | ${sig.slice(0, 20)}...`);
  } catch (e: any) {
    fail('Swap Atomic Buy', e.message?.slice(0, 200));
    if (e.logs) { const el = e.logs.filter((l: string) => l.includes('Error') || l.includes('failed') || l.includes('custom') || l.includes('Program log')); if (el.length) console.log('  Logs:', el.slice(0, 5)); }
  }
  await sleep(DELAY_MS);

  // ===== 17. SWAP ATOMIC SELL (tokens → SOL) =====
  console.log('\n--- Step 16: Swap Atomic Sell ---');
  
  let buyerTokens = 0n;
  try {
    const b = await conn.getTokenAccountBalance(buyerAta);
    buyerTokens = BigInt(b.value.amount);
  } catch { /* */ }
  
  if (buyerTokens === 0n) {
    fail('Swap Atomic Sell', 'No tokens to sell (buy may have failed)');
  } else {
    const sellAmount = buyerTokens / 2n;
    console.log(`  Selling ${sellAmount} raw tokens...`);
    
    try {
      // Step 1: Transfer tokens to vault (with hook accounts)
      let transferIx = createTransferCheckedInstruction(
        buyerAta, mint, tokenVaultPda, mainWallet.publicKey,
        sellAmount, 9, [], TOKEN_2022_PROGRAM_ID
      );
      await addExtraAccountMetasForExecute(
        conn, transferIx, HOOK_PROGRAM_ID,
        buyerAta, mint, tokenVaultPda, mainWallet.publicKey, sellAmount, 'confirmed'
      );

      // Step 2: swap_atomic_sell instruction
      const sellIx = new TransactionInstruction({
        ...
        programId: BOOM_PROGRAM_ID,
        data: Buffer.concat([disc('swap_atomic_sell'), new BN(sellAmount.toString()).toArrayLike(Buffer, 'le', 8), new BN(0).toArrayLike(Buffer, 'le', 8)]),
      });

      const balBefore = await conn.getBalance(mainWallet.publicKey);
      const sig = await sendTx(conn, [transferIx, sellIx], [mainWallet]);
      const balAfter = await conn.getBalance(mainWallet.publicKey);
      const solReceived = (balAfter - balBefore + 5000) / LAMPORTS_PER_SOL;
      pass('Swap Atomic Sell', `received ~${solReceived.toFixed(6)} SOL | ${sig.slice(0, 20)}...`);
    } catch (e: any) {
      fail('Swap Atomic Sell', e.message?.slice(0, 200));
      if (e.logs) { const el = e.logs.filter((l: string) => l.includes('Error') || l.includes('failed') || l.includes('custom') || l.includes('Program log')); if (el.length) console.log('  Logs:', el.slice(0, 5)); }
    }
  }
  ...
  // ===== REPORT =====
  console.log('\n\n🏁 ==========================================');
  console.log('   SWAP E2E TEST RESULTS');
  console.log('==========================================\n');
